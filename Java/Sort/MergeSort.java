public class MergeSort 
{
	...
	private static void merge(int[] a, int[] auxiliaryArray, int low, int mid, int high) {
		for (int i = low; i < high + 1; i++) auxiliaryArray[i] = a[i];
		int j = low;
		int k = mid + 1;
		for (int i = low; i < high + 1; i++) {
			if (j > mid) a[i] = auxiliaryArray[k++];
			else if (k > high) a[i] = auxiliaryArray[j++];
			else if (auxiliaryArray[j] > auxiliaryArray[k]) a[i] = auxiliaryArray[k++];
			else a[i] = auxiliaryArray[j++];
		}
	}
}
